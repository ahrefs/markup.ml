...
let html4_names =
  "lt gt amp quot apos nbsp iexcl cent pound curren yen brvbar sect uml copy \
   ...
   Icirc Iuml ETH Ntilde Ograve Oacute Ocirc Otilde Ouml times Oslash Ugrave \
   ...
   yacute thorn yuml fnof Alpha Beta Gamma Delta Epsilon Zeta Eta Theta Iota \
   Kappa Lambda Mu Nu Xi Omicron Pi Rho Sigma Tau Upsilon Phi Chi Psi Omega \
   alpha beta gamma delta epsilon zeta eta theta iota kappa lambda mu nu xi \
   omicron pi rho sigmaf sigma tau upsilon phi chi psi omega thetasym upsih \
   piv bull hellip prime Prime oline frasl weierp image real trade alefsym \
   larr uarr rarr darr harr crarr lArr uArr rArr dArr hArr forall part exist \
   empty nabla isin notin ni prod sum minus lowast radic prop infin ang and or \
   cap cup int there4 sim cong asymp ne equiv le ge sub sup nsub sube supe \
   oplus otimes perp sdot lceil rceil lfloor rfloor lang rang loz spades clubs \
   hearts diams OElig oelig Scaron scaron Yuml circ tilde ensp emsp thinsp zwnj \
...
let decode text =
  ...
  let rec search copied index =
    if index >= length then
      add_utf_8 buffer text copied (length - copied)
    else if text.[index] <> '&' then
      search copied (index + 1)
    else
    ...
    if start >= length then None
      ...
      if finish > start && finish < length && text.[finish] = ';' then
        Some
          (finish + 1,
           `Codepoint
             (int_of_string (String.sub text start (finish - start))))
      else
        None
