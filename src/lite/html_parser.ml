...
let parse ?depth_limit requested_context report tokens =
  ...
  and push_and_emit ?(formatting = false) ?(acknowledge = false)
    ...
    report_if
      ...
      location
      (fun () -> `Bad_token ("/>", "tag", "should not be self-closing"))
      !throw
      (fun () ->
        ...
        let is_html_integration_point =
          Foreign.is_html_integration_point namespace tag_name attributes
        in

        let attributes =
          List.map (fun (n, v) -> (Namespace.Parsing.parse n, v)) attributes
        in
        let attributes =
          match namespace with
        ...
        emit location
          (`Start_element ((namespace_string, tag_name), attributes))
          mode)
    ...
    | element :: more ->
        emit_text (fun () ->
            (fun k ->
              ...
              else emit_list (Subtree.disable subtree_buffer) k) (fun () ->
  ...
  and pop_implied ?(except = "") location mode =
    pop_until
      (fun { element_name = _, name } ->
        name = except
        || not
           @@ list_mem_string name
                [
                  "dd";
                  "dt";
                  "li";
  ...
  and close_element_with_implied name location mode =
    pop_implied ~except:name location (fun () ->
        ...
        in
        check_element (fun () -> close_element location name mode))
  ...
  and emit_end l =
    pop_until (fun _ -> false) l (fun () -> emit_text (fun () -> !ended ()))
  ...
  and in_body_mode_rules context_name mode = function
    ...
    | l, Start ({ name = "frameset" } as t) ->
        misnested_tag l t context_name (fun () ->
            ...
            | _ ->
                ...
                in
                if not @@ second_is_body !(Stack.elements open_elements) then
                  mode ()
                else if not !frameset_ok then mode ()
                else
    ...
    | l, Start ({ name = "form" } as t) ->
        ...
        then misnested_tag l t "form" mode
        else begin
          close_current_p_element l (fun () ->
    ...
    | l, End { name = "form" } ->
        if not @@ Stack.has open_elements "template" then begin
          ...
          | Some element when Stack.target_in_scope open_elements element ->
              pop_implied l (fun () ->
                  match Stack.current_element open_elements with
                  | Some element' when element' == element -> pop l mode
                  | _ ->
                      report element.location (`Unmatched_start_tag "form")
                        !throw (fun () ->
                          pop_until
                            ...
                            l
                            (fun () -> pop l mode)))
          | _ -> report l (`Unmatched_end_tag "form") !throw mode
        end
    ...
    | l, End { name = ("h1" | "h2" | "h3" | "h4" | "h5" | "h6") as name } ->
        if
          ...
          @@ Stack.one_in_scope open_elements
               [ "h1"; "h2"; "h3"; "h4"; "h5"; "h6" ]
        then report l (`Unmatched_end_tag name) !throw mode
        else
          pop_implied l (fun () ->
              (fun next ->
                match Stack.current_element open_elements with
                | Some { element_name = HTML, name' }
                  when list_mem_string name'
                         [ "h1"; "h2"; "h3"; "h4"; "h5"; "h6" ] ->
                    next ()
                | _ -> report l (`Unmatched_end_tag name) !throw next)
              @@ fun () ->
    ...
    | l, Start ({ name = "rb" | "rtc" } as t) ->
        (fun mode' ->
          let finish () =
            ...
            else misnested_tag l t context_name mode'
          in
  ...
  and adoption_agency_algorithm l name mode =
    ...
    emit_text (fun () ->
        let handled, errors =
          Subtree.adoption_agency_algorithm subtree_buffer
            active_formatting_elements l name
        in
        ...
        | v ->
            ...
            if not only_space then
              let rec reprocess = function
                | [] -> mode ()
                | v :: more ->
                    anything_else_in_table (fun () -> reprocess more) v
              in
    ...
    dispatch tokens begin function
        ...
        | ( l,
            ...
            if
              ...
              @@ Stack.one_in_table_scope open_elements
                   [ "tbody"; "thead"; "tfoot" ]
            then misnested_tag l t "table" in_table_body_mode
            else begin
              push tokens v;
        ...
        | (l, End { name = "table" as name }) as v ->
            if
              ...
              @@ Stack.one_in_table_scope open_elements
                   [ "tbody"; "thead"; "tfoot" ]
            then report l (`Unmatched_end_tag name) !throw in_table_body_mode
            else begin
              push tokens v;
              pop_to_table_body_context l (fun () -> pop l in_table_mode)
            end
        | ( l,
            ...
            report l (`Unmatched_end_tag name) !throw in_table_body_mode
        | v -> in_table_mode_rules in_table_body_mode v
      end
  (* 8.2.5.4.14. *)
  and in_row_mode () =
    dispatch tokens begin function
        ...
        | ( l,
            ...
            report l (`Unmatched_end_tag name) !throw in_row_mode
        | v -> in_table_mode_rules in_row_mode v
      end
  (* 8.2.5.4.15. *)
  and in_cell_mode () =
    dispatch tokens begin function
  ...
  and in_select_in_table_mode () =
    dispatch tokens begin function
        ...
        | v -> in_select_mode_rules in_select_in_table_mode v
      end
  (* 8.2.5.4.18. *)
    ...
    dispatch tokens begin function
        ...
        | l, EOF ->
            (fun mode' ->
              ...
              else mode' ()) (fun () -> emit_end l)
        | l, _ -> report l (`Bad_content "frameset") !throw in_frameset_mode
      end
  (* 8.2.5.4.21. *)
  ...
  and foreign_start_tag mode l tag =
    ...
    push_and_emit ~acknowledge:true ~namespace l tag (fun () ->
        if tag.self_closing then pop l mode else mode ())
        ...
        Start
          ({
             name =
               ...
               | "dd" | "div" | "dl" | "dt" | "em" | "embed" | "font" | "h1"
               | "h2" | "h3" | "h4" | "h5" | "h6" | "head" | "hr" | "i" | "img"
               | "li" | "listing" | "main" | "meta" | "nobr" | "ol" | "p"
               | "pre" | "ruby" | "s" | "small" | "span" | "strong" | "strike"
               | "sub" | "sup" | "table" | "tt" | "u" | "ul" | "var" ) as name;
           } as t) ) as v ->
        ...
        (fun mode' ->
          ...
          | Some { element_name = _, name' }
            when String.lowercase_ascii name' = name ->
              mode' ()
          | _ -> report l (`Unmatched_end_tag name) !throw (fun () -> mode' ()))
          (fun () ->
            let rec scan = function
              | [] -> mode ()
              | { element_name = ns, name' } :: _
