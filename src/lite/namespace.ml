...
module Parsing = struct
  ...
  let push report context raw_element_name raw_attributes throw k =
    let parsed_attributes =
      raw_attributes |> List.map (fun (name, value) -> (parse name, value))
    in
    ...
    let f =
      ...
      |> List.fold_left
           (fun f -> function
             ...
             | ("", "xmlns"), uri -> fun p -> if p = "" then Some uri else f p
             | _ -> f)
           !context.f
    in
        ...
        list_map_cps
          begin fun (name, value) _ k ->
            ...
            | ns, name -> (
                ...
                | None ->
                    report () (`Bad_namespace ns) throw (fun () ->
                        k ((ns, name), value)))
          end
          parsed_attributes throw
          (fun expanded_attributes ->
            k (expanded_element_name, expanded_attributes)))

  let pop ({ contents = { previous } } as context) = context := previous
end
...
module Writing = struct
  ...
  let init top_level =
    let namespace_to_prefix =
      StringMap.empty |> StringMap.add "" [ "" ]
      |> StringMap.add xml_ns [ "xml" ]
      |> StringMap.add xmlns_ns [ "xmlns" ]
    in

    let prefix_to_namespace =
      StringMap.empty |> StringMap.add "" "" |> StringMap.add "xml" xml_ns
      |> StringMap.add "xmlns" xmlns_ns
    in

    let rec entry =
      { namespace_to_prefix; prefix_to_namespace; previous = entry }
    in
  ...
  let lookup report allow_default context namespace throw k =
      ...
      try
        Some
          ...
          |> List.find (fun prefix ->
              ...
              && begin try
                StringMap.find prefix !(fst context).prefix_to_namespace
                = namespace
    ...
    let prefix =
      ...
      | None -> (
          ...
          | Some prefix ->
              if
                ((not allow_default) && prefix = "")
                || StringMap.mem prefix !(fst context).prefix_to_namespace
              then None
              else Some prefix)
    in
  ...
  let format prefix name =
    match prefix with "" -> name | prefix -> prefix ^ ":" ^ name
  ...
  let extend k v map =
    let vs = try StringMap.find k map with Not_found -> [] in
  ...
  let push report context element_name attributes throw k =
    let namespace_to_prefix, prefix_to_namespace =
      attributes
      |> List.fold_left
           (fun (ns_to_prefix, prefix_to_ns) -> function
             | (ns, "xmlns"), uri when ns = xmlns_ns ->
                 (extend uri "" ns_to_prefix, StringMap.add "" uri prefix_to_ns)
           ...
           ( !(fst context).namespace_to_prefix,
             !(fst context).prefix_to_namespace )
    in

    let entry =
      { namespace_to_prefix; prefix_to_namespace; previous = !(fst context) }
    in
    fst context := entry;
    ...
    unexpand_element report context element_name throw (fun element_name ->
        list_map_cps (unexpand_attribute report context) attributes throw
          (fun attributes -> k (element_name, attributes)))
