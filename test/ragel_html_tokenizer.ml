...
let tokenize html : (location * Markup__Html_tokenizer.token) list =
  let ctx = HS.init () in
  let tokens = ref [] in
  ...
  let step = function
    | HS.Text raw ->
      emit (`String (decode raw))
    | HS.Tag (name, attrs) ->
      emit (`Start (tag name (attributes attrs)))
    | HS.Close "br" ->
      (* [HtmlStream] emits both [Tag "br"] and [Close "br"] for [<br/>].
         Feeding the close token to the tree builder creates a second [br]. *)
      ()
    | HS.Close name ->
      emit (`End (tag name []))
